...
- [ ] T014 [P] [US1] Fix the field name in `src/b2aiprep/prepare/resources/feature_schemas/ppgs.json` from `ppg` to `ppgs` to match the parquet column. Delivered by the standalone-defects PR; listed here because US1 cannot pass FR-007 until it lands
- [ ] T015 [P] [US1] Fix the field name in `src/b2aiprep/prepare/resources/feature_schemas/torchaudio_spectrogram.json` from `spectrograms` to `spectrogram`. Delivered by the standalone-defects PR; listed here because US1 cannot pass FR-007 until it lands
...
- [ ] T064 [P] [US4] Apply the T011 decision in `external_scripts/sage_upload_scripts/sage_generate_manifest.py` — include or exclude `provenance/` explicitly. Scope is limited to whether this feature's new files are uploaded; the rest of the Sage upload and verification path is out of scope (FR-028)
- [ ] T065 [P] [US4] Implement release-gating verification of an uploaded copy in `src/b2aiprep/metadata/validate.py` — every output recorded for a distribution present at the publication target, every digest matching, exiting non-zero on any missing file or mismatch, reading a remote inventory via `--inventory`. Do **not** change the exit semantics of `external_scripts/sage_upload_scripts/verify_sage_contents.py`: that script is a diagnostic for deciding whether to re-run an upload or prune a stale cache, and its info-level mismatch reporting is correct for that use (FR-020)
...
- [ ] T084 [P] Remove the `sage_upload_array.sbatch` invocation from `external_scripts/sage_upload_scripts/README.md:54,66-67` and replace it with prose describing what a site-specific array job must do, since `.gitignore:163` excludes `*.sbatch` by policy. Source material is the untracked file's own header: per-subject tasks to avoid folder-creation races, CHUNK subjects per task to stay under the 256-job QOS cap, `%20` concurrency, `--toplevel` for dataset-level files, then verify (constitution IV)
