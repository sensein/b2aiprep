...
- Three live defects found while writing this spec, which stand alone and belong in their own pull request: the two shipped feature dictionaries that name non-existent columns; the sage README's reference to a `.sbatch` that `.gitignore` excludes by policy; and the absence of any documentation for minting or updating the dataset DOI. This feature's validation (FR-007) prevents the dictionary class of defect recurring once it lands.
- Reworking the Sage upload and verification path, beyond deciding whether this feature's own new files are uploaded. `verify_sage_contents.py` is a diagnostic for deciding whether to re-run an upload or prune a stale cache; its info-level mismatch reporting is correct for that purpose, and release gating belongs in this feature's own validator instead.
- Renaming `ppgs.parquet` to `ppgs_ppg.parquet` to fit the `{package}_{feature}` convention. Worth doing at a major release boundary — it renames a column published immutably in 3.0.0, and it needs a payload-shape change rather than a label change, since `bundle_data.py:139-142` resolves `features[feature_class][feature_name]` while extraction stores the tensor at `features['ppgs']`.
