...
from b2aiprep.prepare.constants import RepeatInstrument, Instrument, PARTICIPANT_ID_TO_REMOVE, AUDIO_FILESTEMS_TO_REMOVE
...
class BIDSDataset:
    ...
    def load_phenotype_data(self, phenotype_name: str) -> t.Tuple[pd.DataFrame, t.Dict[str, t.Any]]:
        """
        ...
        df = pd.read_csv(self.data_path.joinpath(f"{phenotype_name}.tsv"), sep="\t")
        with open(self.data_path.joinpath(f"{phenotype_name}.json"), "r") as f:
        ...
        if df.shape[1] > 0 and df.columns[0] == 'record_id' and 'record_id' not in list(phenotype.keys()):
            phenotype = self._add_record_id_to_phenotype(phenotype)
    ...
    def _deidentify_phenotype(self, df: pd.DataFrame, phenotype: dict) -> t.Tuple[pd.DataFrame, dict]:
        """
        ...
        """
        # Remove hard-coded participants
        if "record_id" in df.columns:
            idx = df["record_id"].isin(PARTICIPANT_ID_TO_REMOVE)
            if idx.sum() > 0:
                logging.info(f"Removing {idx.sum()} hard-coded participants.")
                df = df.loc[~idx]

        # Reduce ID length (remap IDs)
        if "record_id" in df.columns:
            df = self._reduce_id_length(df, "record_id")
        if "session_id" in df.columns:
            df = self._reduce_id_length(df, "session_id")

        # Rename record_id to participant_id
        if "record_id" in df.columns:
            df, phenotype = self._rename_record_id_to_participant_id(df, phenotype)
    ...
    def _remove_low_utility_columns(self, df: pd.DataFrame, phenotype: dict) -> t.Tuple[pd.DataFrame, dict]:
        ...
        return self._drop_columns_from_df_and_data_dict(
            df, phenotype, columns_to_drop, "Removing low utility columns"
        )
    ...
    def _add_record_id_to_phenotype(self, phenotype: dict) -> dict:
    ...
    def _rename_record_id_to_participant_id(self, df: pd.DataFrame, phenotype: dict) -> t.Tuple[pd.DataFrame, dict]:
        """Rename record_id column to participant_id."""
        phenotype_updated = {}
        for c in df.columns:
            if c == 'record_id':
                phenotype_updated['participant_id'] = phenotype[c]
            else:
    ...
    def _reduce_id_length(self, df: pd.DataFrame, id_name: str) -> pd.DataFrame:
    ...
    def deidentify(self, outdir: t.Union[str, Path], skip_audio: bool = False) -> 'BIDSDataset':
        """
        ...
        df, phenotype = self.load_phenotype_data("participants")
        df, phenotype = self._deidentify_phenotype(df, phenotype)
        ...
        if phenotype_base_path.exists():
            ...
            for phenotype_filepath in phenotype_base_path.glob("*.tsv"):
                ...
                # Create temporary BIDSDataset for the phenotype subdirectory
                phenotype_dataset = BIDSDataset(phenotype_base_path)
                df_pheno, phenotype_dict = phenotype_dataset.load_phenotype_data(phenotype_filepath.stem)
                df_pheno, phenotype_dict = phenotype_dataset._deidentify_phenotype(df_pheno, phenotype_dict)
                df_pheno, phenotype_dict = phenotype_dataset._clean_phenotype_data(df_pheno, phenotype_dict)
        ...
        if not skip_audio:
            ...
            self._deidentify_audio_files(outdir)
    ...
    def _deidentify_audio_files(self, outdir: Path):
        """
        ...
        # Placeholder for future audio deidentification processing
